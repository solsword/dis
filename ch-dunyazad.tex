...
\Footnotetext{~}{Note: a much less detailed description of how \dunyazad/ works has been published as \citep{Mawhorter2015a}.}
...
%
Potentials include things like \prq{injured}{,} and \prq{threatening}{,} and each is classified as either a ``problem'' or an ``opportunity.''
%
...
%
Additionally, extra predicates specify whether a potential is \prq{problematic\_for}{} one or more of the characters involved (for example, the \prq{injured}{} state is \prq{problematic\_for}{} any character to which it applies).
...
\Cref{fig:dunyazad-state-example} is not quite accurate, because \dunyazad/'s non-surface state predicates are never encountered alone.
%
Instead, they are always wrapped in \prq{st(<T>, <state>)}{} predicates, to define different states of the world for each timepoint $T$ in the story.
%
The timepoints in a story form a directed acyclic graph that branches out from a single root node.
%
This graph is defined using \prq{successor(<previous>, <option>, <next>)}{} predicates which specify that the result of choosing option \prq{<opt>}{} at timepoint \prq{<previous>}{} is the state designated \prq{<next>}{.}
%
...
Representation:\svind \\
\ind \parbox{0.95\linewidth}{ \tt \raggedright
...
\ind inst(actor, businessperson\_4))\cg{).} \vspace{\baselineskip} \\


\cg{at(root,} outcome(option(1), o(attitude, convinced))\cg{).} \\
\cg{at(root,} outcome(option(1), o(is\_enraged, not\_enraged))\cg{).} \vspace{\baselineskip} \\
...
\ind \ind inst(actor, tough\_3), inst(actor, you)))\cg{).} \vspace{\baselineskip} \\
...
\begin{enumerate}
  \item The target is persuaded to calm down (\prq{o(attitude, convinced)}{} and \prq{o(is\_enraged, not\_enraged)}{}), in which case they stop threatening whoever they were threatening.
  \item The target cannot be persuaded to calm down, so they continue threatening their target (\prq{o(attitude, unconvinced)}{} along with \prq{o(is\_enraged, not\_enraged)}{}).
  \item The target is unconvinced, and furthermore gets mad at the person who tried to convince them (\prq{o(attitude, unconvinced)}{} along with \prq{o(is\_enraged, enraged)}{}).
\end{enumerate}
...
As mentioned above, not every timepoint includes state changes that are induced by a setup: they only occur for the \prq{root}{} node and for timepoints that follow \prq{travel\_onwards}{} actions.
%
...
\dunyazad/'s aesthetic constraints go beyond the basics required to produce actions that make sense and can be read as a story.
%
...
Each of the seven steps of choice analysis described in \cref{sec:goal-based-choice-analysis} has a corresponding set of rules in \dunyazad/ which are defined using the rules for earlier steps:
%
...
\end{itemize}
%
The predicates created by \file{goals.lp} include \prq{at(\abr/<T>,\abr/ player\_goal(\abr/<goal>))}{} predicates, which specify the goals of the player for each timepoint, and \prq{at(\abr/<T>,\abr/ goal\_stakes(\abr/<goal>,\abr/ <priority>))}{} predicates, which assign priorities to each goal.
...
%
This step produces the \prq{at(\abr/<T>,\abr/ likely\_outcome(\abr/<option>,\abr/ <outcome>))}{} and \prq{at(\abr/<T>,\abr/ unlikely\_outcome(\abr/<option>,\abr/ <outcome>))}{} predicates.
%
...
%
First, all possible outcome components of an option are considered and if any of them negatively impacts a goal, that option \lbl{threatens} that goal.
%
Similarly, any positive impact entails an \lbl{enables} relationship.
%
...
%
For the \lbl{advances} and \lbl{hinders} evaluations the same logic applies, but considering only outcome components marked as \prq{likely}{}.
%
The result of the option analysis constraints is a set of \prq{at(\abr/<T>,\abr/ expectation(\abr/<option>,\abr/ <expectation>,\abr/ <goal>))}{} predicates that list the expectations for each option/goal pair.
%
\Cref{fig:dunyazad-option-analysis-example} shows an example of these \prq{expectation}{} predicates for the same option whose likelihood predicates are shown in \cref{fig:dunyazad-likelihood-analysis-example}.

\item %
\textbf{Relative Option Analysis} \\
%
Integrating goal priorities and option expectations, relative option analysis produces \prq{at(\abr/<T>,\abr/ option\_feel(\abr/<option>,\abr/ <feel>))}{} predicates, which collapse the various expectations across goals at each option to produce a general evaluation of that option.
%
...
The \prq{option\_structure}{} assignments derived from these \prq{option\_feel}{} labels are the subject of the experiment described in \cref{ch:option-results} and provide one primary means of controlling \dunyazad/'s output.
%
...
\Cref{fig:dunyazad-relative-option-analysis-example} shows an example of relative option analysis results.
%
Based on having one \prq{hopeful}{} option and one option which is both \prq{bad}{} and \prq{doomed}{,} the choice in question is both \prq{obvious}{} and \prq{pressured}{} from a prospective standpoint.
%
...
%
The \prq{pressured}{} label applies to choices where there are neither \prq{safe}{} nor \prq{irrelevant}{} options but there is at least one \prq{hopeful}{} or \prq{tradeoff}{} option, and it indicates a choice where risk is inevitable but success still seems possible.
%
Note that exact definitions for these labels can be found in the file \file{eval.lp} listed in \cref{sec:src-core}.
...
\begin{hangparas}{1.3em}{1}
  \setlength{\parskip}{0pt}
1. You try to talk the bandits down (You have skill: negotiation and you have skill: storytelling). \\
\ind \parbox{0.88\linewidth}{$\rightarrow$ You talk to the bandits and convince them to back off. \newline The bandits are no longer threatening the merchant.}
...
%
These are represented using \prq{at(<T>, outcome\_perception(<option>, <perception>, <goal>))}{} predicates and have four basic \prq{<perception>}{} values, similar to the \prq{expectation}{} predicates from option analysis: \prq{good\_for}{,} \prq{bad\_for}{,} \prq{great\_for}{,} and \prq{awful\_for}{.}
%
...
%
So for example, the \prq{o(\abr/success,\abr/ dispelled)}{} outcome component of the \prq{dispel}{} action has a consequence which removes the \prq{polymorphed}{} property from the action's target.
%
Accordingly, any option that consists of a \prq{dispel}{} action with a polymorphed target will be \prq{good\_for}{} the \prq{preserve\_\abr/original\_\abr/form}{} goal of its target, (because the \prq{polymorphed}{} state which is being removed is \prq{awful\_for}{} the \prq{preserve\_\abr/original\_\abr/form}{} goal\footnote{%
...
%
In \dunyazad/, valence is represented by \prq{at(\abr/<T>,\abr/ outcome\_overall(\abr/<option>,\abr/ <evaluation>\abr/))}{} predicates, while \prq{at(\abr/<T>,\abr/ overall\_predictability(<option>,\abr/ <predictability>))}{} predicates represent predictability.
%
The predictability of individual outcome components is represented by \prq{at(\abr/<T>,\abr/ outcome\_predictability(\abr/<option>,\abr/ <outcome>, <predictability>))}{} predicates.
%
...
%
\item \prq{expected}{}---An outcome component is \prq{expected}{} when it is one of several likely values for its variable, at least one which is important.
%
An entire option is \prq{expected}{} when every outcome at that option is either \prq{predictable}{,} \prq{expected}{,} or \prq{average}{,} and they're not \emph{all} \prq{average}{.}
%
...
{
\centering
\begin{tabular}{p{9.5em} p{6.5em} p{8em} p{7.5em}}
\toprule
...
%
These are represented using \prq{at(<T>, outcome\_feel(<option>, <feel>))}{} predicates.
%
