...
%
The goal of this step is to identify outcome components that the player can presume will result from each option, so if there are actually multiple conflicting likely scenarios, none of them counts as \lbl{likely}.
%
...
%
Using the same procedure, but this time only considering \lbl{likely} outcomes, \lbl{advances} and \lbl{hinders} option expectations are assigned to each option/goal pairing (these are the `*' and `!' symbols in \cref{fig:choice-analysis-method}).
%
...
%
All of this information (besides the balance of impacts) is directly encoded in the option expectations in the form of \lbl{hinders} labels, so classic dilemmas are easy to identify using this analysis method.
%
...
%
The labels in \cref{tab:prospective-impressions} and their criteria were developed as part of \dunyazad/'s choice analysis component, and some of them (the \lbl{depressing}, \lbl{empowering}, \lbl{obvious}, and \lbl{relaxed} labels) were refined based on experimental results (see \cref{ch:results}).
...
%
First, however, each outcome component is established as either \lbl{important} or \lbl{unimportant} based on whether or not it affects any high-priority player goals (using actual effects, not merely indicated effects).
%
...
%
\Cref{ch:results} includes some updates to the material presented here (mainly \cref{tab:prospective-impressions,tab:retrospective-impressions}; see \cref{sec:results-prospective-caveats}), as these refinements make the most sense when presented alongside the experimental results that led to them.
